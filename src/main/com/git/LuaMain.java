...
public class LuaMain {
    ...
    public static void main(String[] args) throws NoSuchMethodException, IllegalAccessException {
        ...
        try (var arena = Arena.ofConfined()) {
            ...
            //lua_h.lua_pushcclosure(lua_State, callback, 0);
            // 这里虽然指定了callback，但是errorFun指定可能有问题，目前无法回到错误函数
            lua_h.lua_pushcclosure(lua_State, lua_CFunction.allocate(new My_Lua_CFunction(), Arena.ofAuto()), 0);
            lua_h.lua_setglobal(lua_State, arena.allocateFrom("upcall_java"));

            lua_h.lua_pushcclosure(lua_State, lua_CFunction.allocate(new My_Lua_CFunction2(), Arena.ofAuto()), 0);
            lua_h.lua_setglobal(lua_State, arena.allocateFrom("upcall_java_2"));



            //lua_h.lua_callk(lua_State, 0, 1, 0, callback);
    ...
    static class My_Lua_KFunction implements lua_KFunction.Function {
        ...
        public int apply(MemorySegment lua_State, int status, long ctx) {
            ...
            int top = lua_h.lua_gettop(lua_State);
    ...
    static class My_Lua_CFunction2 implements lua_CFunction.Function {
        ...
        public int apply(MemorySegment lua_State) {
            var param1 = lua_h.lua_tointegerx(lua_State, 1, NULL);
            var data2 = lua_h.lua_tolstring(lua_State, 2, NULL);
            var param2 = data2.getString(0L);
            var result = param2 + param1;
            var arena = Arena.ofAuto();
            lua_h.lua_pushlstring(lua_State, arena.allocateFrom(result), result.getBytes().length);
            return 1;
        }
    }
    ...
    static class NewUserCFunction implements lua_CFunction.Function {
        ...
        public int apply(MemorySegment lua_State) {
            ...
            lua_h.lua_pushinteger(lua_State, 5);
            return 1;
        }
    }
